...
namespace Watermelon
{
    public class LivesManager : MonoBehaviour
    {
        ...
        private static void SetLifes(int value)
        {
            Debug.Log($"[LivesManager] SetLifes called - Old Value: {save.livesCount}, New Value: {value}");
        ...
        public static void RemoveLife()
        {
            ...
            Debug.Log($"[LivesManager] RemoveLife called - Before: {Lives}");
            ...
            Debug.Log($"[LivesManager] RemoveLife called - After: {Lives}");
        ...
        public static void AddLife()
        {
            Debug.Log($"[LivesManager] AddLife called - Before: {Lives}");
            
            if (Lives < instance.data.maxLivesCount)
                Lives++;
            
            Debug.Log($"[LivesManager] AddLife called - After: {Lives}");
        }
        ...
        private IEnumerator LivesCoroutine()
        {
            ...
            while (Lives < data.maxLivesCount)
            {
                ...
                if (timespan >= oneLifeSpan)
                {
                    Debug.Log($"[LivesManager] LivesCoroutine - Restoring 1 life. Current: {Lives}");
                    Lives++;
        ...
        private class LivesSave : ISaveObject
        {
            ...
            [SerializeField] private long dateBinary;
            public DateTime date;
            ...
            public void Init(LivesData data)
            {
                Debug.Log($"[LivesSave] Init called - firstTime: {firstTime}, livesCount: {livesCount}, infiniteLives: {infiniteLives}");
                ...
                if (firstTime)
                {
                    ...
                    Debug.Log($"[LivesSave] First time init - Setting lives to max: {livesCount}");
                }
                else
                {
                    date = DateTime.FromBinary(dateBinary);

                    Debug.Log($"[LivesSave] Loading saved data - Lives: {livesCount}, Date: {date}");
                    ...
                    if (livesCount < data.maxLivesCount)
                    {
                        ...
                        Debug.Log($"[LivesSave] Restored {restoredLives} lives. Current lives: {livesCount}");
                    }
                }
            }
            ...
            {
                dateBinary = date.ToBinary();
                Debug.Log($"[LivesSave] Flush called - Saving Lives: {livesCount}, InfiniteLives: {infiniteLives}, Date: {date}");
            }
        }
