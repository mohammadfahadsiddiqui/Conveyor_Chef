    ...
    public class PrivacyManager : MonoBehaviour
    {
        ...
        public async Task CheckConsents()
        {
            ...
            if (!PlayerPrefs.HasKey(FirstLaunchKey))
            {
                ...
                if (needGdprConsent)
                {
                    await  OpenPrivacyScreen(countryCode);
                }
                else
                {
        ...
        private async Task<bool> CheckGdprApiForRequirement()
        {
            ...
            countryCode = consentInfo.country_code;
            return consentInfo.is_gdpr;
        }

        public async Task OpenPrivacyScreen(string currentCountryCode="EN")
        {
            var privacyScreen = Instantiate(privacyScreenPrefab).GetComponent<PrivacyScreenBehaviour>();
            privacyScreen.Localize(countryCode);
            DontDestroyOnLoad(privacyScreen.gameObject);
            privacyScreen.ConfirmWaitTask = new TaskCompletionSource<bool[]>();
            if (PlayerPrefs.HasKey(FirstLaunchKey))
            {
                privacyScreen.SetToggleStates(AdConsent, AnalyticsConsent);
            }
        ...
        {
            var privacyPartnerScreen = Instantiate(privacyPartnersScreenPrefab).GetComponent<PrivacyPartnersScreenBehaviour>();
            privacyPartnerScreen.Localize(countryCode);
        }
    }
}
